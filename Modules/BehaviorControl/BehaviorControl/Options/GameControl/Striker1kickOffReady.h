option(Striker1kickOffReady)
{
  ...
  state(positiononfield)
  {
	  transition
	  {
		  if((std::abs(theRobotPose.translation.x() + 300) < 100 &&std::abs(theRobotPose.translation.y()) < 100 && std::abs(theRobotPose.rotation) < 5_deg) || state_time>15000)
			  goto wait;
	  }
	  action
	  {
		  WalkToTarget(Pose2f(30.f, 30.f, 30.f),Pose2f(-theRobotPose.rotation,-(theRobotPose.translation.x() + 300) ,-(theRobotPose.translation.y())));
		  LookLeftAndRight();
