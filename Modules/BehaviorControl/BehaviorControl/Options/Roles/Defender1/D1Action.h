...
//                    goto turnToTeamBallL;
           ...
           else  if(theTeamBallModel.position.x()<0)  //如果球在对方半场
                {
                    if(theLibCodeRelease.timeSinceBallWasSeen < 400 ) //如果自己看到了球 
                        {
                            if(theLibCodeRelease.defender2.isPenalized || !theLibCodeRelease.defender2.isUpright)  //如果S2不在线，则直接开始进攻
                                    goto attack;
                            
                            if( theFrameInfo.getTimeSince(theLibCodeRelease.defender2.theBallModel.timeWhenLastSeen) < 500 &&
                                    ...
                                    std::pow(theTeamBallModel.position.y()-theLibCodeRelease.defender2.theRobotPose.translation.y(),2)) < 100)  //球离D1的距离-球到D2的距离)
                                        goto attack;
                            if(theFrameInfo.getTimeSince(theLibCodeRelease.defender2.theBallModel.timeWhenLastSeen) > 500)  //D2没有看到球
                                        goto attack;
                            goto waitForBall;
                        }
                    ...
                    if(theLibCodeRelease.timeSinceBallWasSeen > 400 )  //如果没有看到球
                        {
                            if(!theLibCodeRelease.defender2.isPenalized && theLibCodeRelease.defender2.isUpright &&    //如果S2在线
                                  theFrameInfo.getTimeSince(theLibCodeRelease.defender2.theBallModel.timeWhenLastSeen) < 500 &&  //S2看到了球
                                 (float)std::sqrt(std::pow(theTeamBallModel.position.x()-theLibCodeRelease.defender2.theRobotPose.translation.x(),2) +
                                    std::pow(theTeamBallModel.position.y()-theLibCodeRelease.defender2.theRobotPose.translation.y(),2)) < 600)  //S2离球的距离小于600，证明S2要attack了
                                {
  ...
  state(waitForBall)
  {
      transition
      {
          OUTPUT_TEXT("D1Action_waitForBall");
          if(state_time>1000)
              goto start;
      }
      action
  ...
  state(waitForSelfBall)
  {
      transition
      {
          OUTPUT_TEXT("D1Action_waitForSelfBall");
          if(state_time>1000)
              goto start;
      }
      action
 ...
 state(turnToBall)
  {
    transition
    {
	  OUTPUT_TEXT("D1Action_turnToBall");
