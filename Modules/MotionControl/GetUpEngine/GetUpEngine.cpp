...
{
    //OUTPUT_TEXT(static_cast<int>(theDamageConfigurationBody.getUpBack.size()));
    //OUTPUT_TEXT(Blackboard::getInstance().exists("DamageConfigurationBody"));
    //OUTPUT_TEXT(lineCounter);
#ifndef NDEBUG
  //OUTPUT_TEXT("GetUpEngine::update()_NDEBUG");
  ...
  {
      //static int jjj=0;
    //OUTPUT_TEXT("GetUpEngine::update()_if");
    if(!wasActive)
    {
      //OUTPUT_TEXT("GetUpEngine::update()_if_!wasActive");
      wasActive = true;
      output.tryCounter = 0;
      ...
      {
          //OUTPUT_TEXT("GetUpEngine::update()_if_!wasActive_if");
          //jjj=1;
          state = schwalbe;
      }
    }
    ...
    {
         //OUTPUT_TEXT("GetUpEngine::update()_if_state == decideAction ");
         pickMotion(output); //this decides what is to do
    }

    if(state == working)
    {
      //OUTPUT_TEXT("GetUpEngine::update()_if_state == working ");
      setNextJoints(output);
      if(lineCounter >= maxCounter)
      {
        //OUTPUT_TEXT("GetUpEngine::update()_if_state == working_if");
        if(verifyUprightTorso(output))
        {
          //OUTPUT_TEXT("GetUpEngine::update()_if_state == working_if_if");
    ...
    {
      //OUTPUT_TEXT("GetUpEngine::update()_elseif_state == recover");
      setNextJoints(output);
      if(lineCounter >= maxCounter)
      {
        state = decideAction;
    ...
    {
      //OUTPUT_TEXT("GetUpEngine::update()_elseif_state == pickUp");
      setNextJoints(output);
      if(lineCounter >= maxCounter)
        output.isLeavingPossible = true;
    }
  ...
  {
    //OUTPUT_TEXT("GetUpEngine::update()_else");
...
//    OUTPUT_TEXT(bodyAngleX);
//    OUTPUT_TEXT(bodyAngleY);
    //OUTPUT_TEXT(output.tryCounter);
  ...
  switch(state)
  {
    case decideAction:
    {
        //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_decideAction");
      if(bodyAngleY > 65_deg && theFrameInfo.getTimeSince(lastNotActiveTimeStamp) > 300)
      {
          //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_decideAction_if");
        //init stand up front
        if(output.tryCounter >= static_cast<int>(theDamageConfigurationBody.getUpFront.size()))
        {
            //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_decideAction_if_if");
            //iii=1;
            state = schwalbe;
        }
        else
        {
            //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_decideAction_if_else");
          state = working;
      ...
      {
          //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_decideAction_elseif1");
        //init stand up back motion
        state = working;
        if(output.tryCounter >= static_cast<int>(theDamageConfigurationBody.getUpBack.size()))
        {
             //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_decideAction_elseif1_if");
             //iii=2;
            state = schwalbe;
        }
      ...
      {
          //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_decideAction_elseif2");
        //init stand
        state = pickUp;
      ...
      {
          //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_decideAction_else");
    ...
    case breakUp:
    {
      ...
      {
          // OUTPUT_TEXT("GetUpEngine::pickMotion()_state_breakUp_else");
        //init recover motion
        state = recover;
    ...
    case schwalbe:
    {
         //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_schwalbe");
      if(abs(theInertialData.angle.y()) > 35_deg) //not near upright
      {
          //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_schwalbe_if");
      ...
      {
          //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_schwalbe_else");
    ...
    {
        //OUTPUT_TEXT("GetUpEngine::pickMotion()_state_default");
      setCurrentMotion(GetUpMotions::numOfGetUpMotions);
      state = decideAction;
      break;
    }
